...
def calculate_Exch(positions, Acc,Aoo,Aco,Aoc, Bcc,Boo,Bco,Boc):
	...
	for i in range(0,N//2,1):
		for j in range(i+1,N//2,1):
			...
			forces[2*j] += F
			# O-O
			E, F           = V_exch(positions[2*j+1] - positions[2*i+1], Aoo, Boo)
			Eoo           += E
			forces[2*i+1] -= F
			forces[2*j+1] += F
			# C-O
...
class MvH_CO(Calculator):
	...
	as described in
		...
		ApJ 2013, 781 (1), 16.
		https://doi.org/10.1088/0004-637X/781/1/16.
	...
	parameters_FS = {   'epsilon':   11.230,         # epsilon: Absolute minimum depth, default 11.230
						'rho0'   :    2.3281*1.1282, # Exponential prefactor.
						'r0'     :    1.1282,        # r0: Minimum distance, default 1.1282
						'Ccc'    :  -33.37,	         # Cij : Dispersion coefficients in eV*Angstrom^6
						'Coo'    :  -10.52,
						'Cco'    :  -15.16,
						'Coc'    :  -15.16,
						'Acc'    :  361.36,
	...
	parameters_runtime = { \
	  "COCO_Q_VARIABLE" : True,		# if True, use variable charges
	  "COCO_CMS_WEIGHTS" : False,	# if True, use center-of-mass weights from coco
	  "COCO_FQ_MODE" : False,		# if True, do not include force contributions due to CO-bond-length-dependent charges
	  "COCO_FX_MODE" : True			# if True, do not add torque-free projection of forces on X to C and O
	}
	...
	def calculate(self, atoms=None, properties=['energy'],
		...
		E_Morse, F_Morse = calculate_Morse(positions,
										   **{k:self.parameters[k] for k in ('epsilon','rho0','r0')})
		#print("Morse : E = %+.6e eV  |F| = %.6e" % (E_Morse,np.linalg.norm(F_Morse)) )

		E_Exch, F_Exch = calculate_Exch(positions,
										**{k:self.parameters[k] for k in ('Acc','Aoo','Aco','Aoc', 'Bcc','Boo','Bco','Boc')})
		E_Disp, F_Disp = calculate_Disp(positions,
		...
		E_Coul, F_Coul              = calculate_Coul(positions, cms_weights, **Coul_kwargs)
