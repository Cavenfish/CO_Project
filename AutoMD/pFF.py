...
def getInteraction(df, mList, comb, positions):
    for pair in comb:
        i = pair[0]
        j = pair[1]
        ...
        df['DispE']                += E
        df['DispF'][2*i]           -= (Fcc + Foc)
        df['DispF'][2*i+1]         -= (Foo + Fco)
        df['DispF'][2*j]           += (Fcc + Fco)
        df['DispF'][2*j+1]         += (Foo + Foc)

        #Calculate Exchange
        E, Fcc, Foo, Fco, Foc = calculate_Exch(rj0i0, rj1i1, rj0i1, rj1i0)
        df['ExchE']                += E
        df['ExchF'][2*i]           -= (Fcc + Foc)
        df['ExchF'][2*i+1]         -= (Foo + Fco)
        df['ExchF'][2*j]           += (Fcc + Fco)
        df['ExchF'][2*j+1]         += (Foo + Foc)
        ...
        # print(df['CoulF'][2*j])
        # print(Fj0)
        # print(df['CoulF'][2*j] + Fj0)
    return


def executeCalculations(positions, comb, numProcs):

    #Initialize variables and shared dictionary
    epsilon  = 11.230139012256362
    numPairs = len(comb)
    N        = len(positions)
    manager  = Manager()
    ...
    #Fill shared dictionary with blanks
    for i in ['MorseF', 'ExchF', 'DispF', 'CoulF']:
        df[i] = manager.list(zeros_like(positions))
    #Fill shared dictionary with blanks
    for i in ['MorseE', 'ExchE', 'DispE', 'CoulE']:
        df[i] = 0.0

    n     = numPairs // numProcs
    m     = numProcs - 1
    procs = []
    args  = (df, mList, comb[n*m:], positions)
    proc  = Process(target=getInteraction, args=args)
    procs.append(proc)
    proc.start()
    for i in range(m):
        args = (df, mList, comb[n*i : n*(i+1)], positions)
        proc = Process(target=getInteraction, args=args)
        procs.append(proc)
        proc.start()
    ...
    # the following energy will be added to change the zero point
    # of all intramolecular interactions.
    df['MorseE'] += epsilon * N / 2.0

    # Morse = (MorseE, MorseF)
    # Exch  = ( ExchE,  ExchF)
    # Disp  = ( DispE,  DispF)
    # Coul  = ( CoulE,  CoulF)
    return df#Morse, Exch, Disp, Coul
...
class MvH_CO(Calculator):
    ...
    def __init__(self, numProcs=1, **kwargs):
    ...
    def calculate(self, atoms=None, properties=['energy'],
        ...
        if comb.size == 0:
            ...
            df = executeCalculations(pos, comb, self.numProcs)
            Morse = (df['MorseE'], array(df['MorseF']))
            Exch  = ( df['ExchE'],  array(df['ExchF']))
            Disp  = ( df['DispE'],  array(df['DispF']))
            Coul  = ( df['CoulE'],  array(df['CoulF']))
